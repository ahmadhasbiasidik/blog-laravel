    ...
    <form action="/dashboard/posts" method="post" class="mb-5" enctype="multipart/form-data">
        ...
        <div class="mb-3 d-flex justify-content-between">
            ...
            <button type="submit" class="btn btn-sm btn-default border">
                <span data-feather="save"></span> Create Post
            </button>
        </div>
        <div class="mb-3">
            <label for="title" class="form-label">Title</label>
            <input type="text" name="title" class="form-control @error('title') is-invalid @endError" id="title" placeholder="Your title" value="{{ old('title') }}" required autofocus>
            @error('title')
        ...
        <div class="mb-3">
            ...
            <input type="text" name="slug" class="form-control @error('slug') is-invalid @endError" id="slug" placeholder="Your slug" readonly value="{{ old('slug') }}" required>
            @error('slug')
                <div class="invalid-feedback">{{ $message }}</div>
            @endError
        </div>
